...
Provides type-specific validation for extractors, converters, templates, and filters.
"""
...
def validate_filter_function(function: Callable) -> ValidationResult:
    ...
    Expected signature: def filter_files(context: ProcessingContext, *args, **kwargs) -> bool
    Expected return: Boolean indicating whether file should be processed
    """
    try:
        sig = inspect.signature(function)
        params = list(sig.parameters.values())
        ...
        if sig.return_annotation != sig.empty:
            if 'bool' in str(sig.return_annotation).lower():
                ...
                validation_points.append("⚠ Return type should be bool (True = include file)")
        else:
...
def validate_allinone_function(function: Callable) -> ValidationResult:
    ...
    Expected signature: def process_file(context: ProcessingContext, *args, **kwargs) -> Dict[str, Any]
    Expected return: Dictionary with 'formatted_name' key (handles extraction + conversion + formatting)
    """
    try:
        sig = inspect.signature(function)
        params = list(sig.parameters.values())
        ...
        if sig.return_annotation != sig.empty:
            if 'dict' in str(sig.return_annotation).lower() or 'Dict' in str(sig.return_annotation):
                ...
                validation_points.append("⚠ Return type should be Dict[str, Any] with 'formatted_name'")
        else:
            validation_points.append("ℹ No return type annotation (should return Dict with 'formatted_name')")
        
        additional_params = params[1:]
        
        message = f"All-in-One: {function.__name__}()\n" + "\n".join(validation_points)
        message += "\n\nShould return: {{'formatted_name': 'final_filename'}} (complete processing)"
